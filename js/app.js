...
function validarBusqueda(e) {
  ...
  if (busqueda.length < 3) {
    mostrarMensaje("Búsqueda muy corta... Añade mas información, por favor");
    return;
  }
...
function mostrarMensaje(msg) {
  ...
  if (!alertaPrevia) {
    ...
    (alerta.textContent = msg), formulario.appendChild(alerta);
...
function mostrarVacantes (vacantes) {
    ...
    if(vacantes.length > 0) {
        ...
        vacantes.forEach( vacante => {
            const{ company, title, url,type, description } = vacante;
